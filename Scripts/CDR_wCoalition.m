...
Participants = input("No. of desired participants (6, 10, 20, 50, 500, 5000, 10000): " ); %input number of participants
...
elseif Participants == 16000
    x = '16k-participantsTRCase2.xlsx';
else
...
if scen == 3 || scen == 6
    ...
    figure(4)
    hold on
    stairs(time.',TotalGen);
    stairs(time.',TotalLoad);
    hold off
...
elseif scen == 3 || scen == 6;
  ...
  if y_DR < no_DR
      ...
      fprintf("ACDR: %f\n", before)
      fprintf("Unfulfilled ACDR: %f\n", after)
      fprintf("Fulfilled ACDR: %f \n", before-after)
  elseif y_DR > no_DR
      fprintf("\nTherefore, the DR event worsened the system imbalance\n")
  else
       fprintf("\nTherefore, the DR event neither improved nor worsened the system imbalance\n")
...
for z=1:length(avail_grp)
    ...
    mem(z,1) = nega;
    mem(z,2) = posi;
    capacity(z,1) = abs(cap_nega);
    capacity(z,2) = cap_posi;
end
...
figure(12)
    hold on
    plot(timed,sum(final,2));
    plot(timed,sum(P_b,2));
    patch([timed(:); flip(timed(:))], [sum(final,2); flip(sum(P_b,2))], 'b', 'FaceAlpha',0.25, 'EdgeColor','none')
    ...
    hold on
    plot(timed,sum(final,2));
    plot(timed,sum(AfterDR_Final_indiv,2));
    patch([timed(:); flip(timed(:))], [sum(final,2); flip(sum(AfterDR_Final_indiv,2))], 'b', 'FaceAlpha',0.25, 'EdgeColor','none')
...
function [coalition_pi] = coalition_pi(P_b, AfterDR, final, group_mem)
    ...
    n = area(ideal,no_resp);
    y = area(ideal,with_resp);
