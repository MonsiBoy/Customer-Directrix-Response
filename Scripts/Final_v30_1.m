...
if scen == 3 || scen == 6
    figure(1)
    hold on
    stairs(timed,(sum(final,2))/1000000);
    stairs(timed,(sum(AfterDR_Final_indiv,2))/1000000);
    stairs(timed,sum(P_b,2)/1000000);
    hold off
    ...
    figure(4)
    hold on
    stairs(time.',TotalGen/1000000);
    stairs(time.',TotalLoad/1000000);
    hold off
...
%% Plot Difference
final_kW = sum(final,2)/1000000;
afterdr_kW = sum(AfterDR_Final_indiv,2)/1000000;
beforedr_kW = sum(P_b,2)/1000000;
figure(12)
    hold on
    plot(timed,final_kW);
